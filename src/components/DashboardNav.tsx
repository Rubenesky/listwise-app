...
const AYUDA_HREF = "/dashboard/help";

// Competencia is intentionally left out of the nav: it's a very-beta
// feature we're not actively developing, so it stays reachable by direct
// URL only, freeing a slot so everything else fits inline on one line.
const primaryNav = [
...
// Mobile has vertical room, so Ayuda rejoins the flat list there — on
// desktop it lives as a "?" icon next to the avatar instead.
const navigation = [
  ...primaryNav.slice(0, 7),
  { name: "❓ Ayuda", href: AYUDA_HREF, download: false },
  primaryNav[7],
];
        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              aria-label="Ayuda"
              title="Ayuda"
              className={`hidden md:flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold shrink-0 ${
                pathname === AYUDA_HREF ? styles.linkActive : styles.linkInactive
              }`}
            >
              ?
            </Link>

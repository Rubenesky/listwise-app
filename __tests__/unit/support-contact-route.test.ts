...
describe("POST /api/support/contact", () => {
  ...
  it("sends via Resend's no-verification sandbox sender to the fixed inbox, with the user's identity in subject/body", async () => {
    const res = await POST(makeRequest({ message: "Necesito ayuda con mi cuenta, gracias" }));
    expect(res.status).toBe(200);
    expect(sendEmail).toHaveBeenCalledWith(
      expect.objectContaining({
        from: "ListWise <onboarding@resend.dev>",
  ...
  it("returns 502 with a real error — not a false success — when the send resolves with success: false", async () => {
    // Realistic failure mode: sendEmail() catches the Resend API error
    // internally and resolves with success: false rather than throwing.
